...
- **`grafico`** = graphic on top: `"impreso"` | `"vinil_corte"` | `"ninguno"`.
  ...
  - `vinil_corte` charges the **cuadro de corte**: ONE single rectangle enclosing ALL design paths (including air between text rows — that's how the owner cuts and charges), clamped to the caja bounds. Cost = lineal meters of roll × `precio_ml` of the selected `vinil_id` from VINILOS (roll width from the vinyl entry — 0.60 m std per the owner; both orientations tried, cheapest wins). Result carries `material_cara["cuadro_corte"]` = `{ancho_cm, alto_cm, area_m2, ml_rollo, vinil_id, vinil_nombre}`. The UI draws the cuadro as a dashed orange rectangle on the preview.
  - `ninguno` — plain face.
...
- `generar_pdf_entrega(result, meta)` — delivery receipt + warranty (Acta de Entrega). **Warranty is 3 months** (`_vence_garantia()`, calendar-safe month math), aligned with the 90-day minimum of art. 77 LFPC; includes claim procedure (written notice + 10 business days response), visible-defects waiver on signature, permits/licencia clause, and jurisdiction (México). Description block is caja-aware (W×H×prof + cuadro de corte via `_dims_caja()` instead of per-letter fields). **Party identification & delivery data**: the acta date is the REAL delivery date (`fecha_entrega` param, default today — never the quote date; warranty runs from it); `lugar` and `anticipo` (real down-payment, default 50%) come from the "Datos de entrega" modal in the UI (acta button opens it). `_meta_con_cliente()` in `main.py` links the client catalog to documents: if the quoted name matches a registered client (`db.get_client_by_name`, case-insensitive), RFC/teléfono/dirección print under EL CLIENTE (clients table has a defensively-migrated `direccion` column). EL PROVEEDOR side prints `catalog_data.EMPRESA` (razon_social/rfc/direccion/telefono/email — editable in `catalog.json` section `"empresa"`; empty fields render as blank lines to fill by hand).
...
**Caja de luz planos** — when `result.tipo == "caja_luz"` the public API dispatches to `_construir_pdf_caja()` instead (the box is ONE piece — never enumerate vinyl letters): drawing with global caja cotas (W top, H left), dashed orange **cuadro de corte** (material reference — its W/H live in the ficha/BOM, not as drawing cotas) plus **position cotas** from the caja edges (left offset in the last bottom row, top offset in the left inner column — so the installer can center the vinyl). **Detail cotas are per RENGLÓN** (`_filas_diseno()` groups design paths by Y-overlap — e.g. big "CFE" row + small "COMISIÓN FEDERAL" row): each row gets its width below (staggered, ≤`MAX_FILAS_COTA`) and its height on the right column (rows never overlap in Y so one column suffices) — a single global cuadro confused the workshop. Lateral ficha (caja/prof/cara/sercha/fondo/cuadro/vinil/posición/renglones/LEDs) and cajetín with `W × H × prof`. Taller page 2 = BOM (caja-aware: "Sercha" label, Gráfico row with cuadro + ml de rollo, Cableado) + ficha de fabricación + cajón side profile. Caja outline in SVG space = closed path whose bbox covers ≥80% of the joint bbox; caja cm dims are solved from `perimetro_total_cm`/`area_cara_cm2` (P=2(w+h), A=w·h).
