...
**Caja de luz planos** — when `result.tipo == "caja_luz"` the public API dispatches to `_construir_pdf_caja()` instead (the box is ONE piece — never enumerate vinyl letters): drawing with global caja cotas (W top, H left), dashed orange **cuadro de corte** with its own W/H cotas plus **position cotas** from the caja edges (left offset bottom row 2, top offset left inner column — so the installer can center the vinyl), lateral ficha (caja/prof/cara/sercha/fondo/cuadro/vinil/posición/LEDs) and cajetín with `W × H × prof`. Taller page 2 = BOM (caja-aware: "Sercha" label, Gráfico row with cuadro + ml de rollo, Cableado) + ficha de fabricación + cajón side profile. Caja outline in SVG space = closed path whose bbox covers ≥80% of the joint bbox; caja cm dims are solved from `perimetro_total_cm`/`area_cara_cm2` (P=2(w+h), A=w·h).
